...
async function loadData() {
    ...
    try {
        if (USE_MOCK_DATA) {
            ...
            const response = await fetch(`${API_BASE_URL}/${currentTimeframe}-metrics`);
...
function updateSummaryCards(data) {
    ...
    // Top Gainer
    const topGainer = [...data].sort((a, b) => b[returnField] - a[returnField])[0];
    document.getElementById('topGainer').textContent = topGainer.symbol;
    document.getElementById('topGainerChange').textContent = `+${topGainer[returnField].toFixed(2)}%`;
    
    // Top Loser
    const topLoser = [...data].sort((a, b) => a[returnField] - b[returnField])[0];
    document.getElementById('topLoser').textContent = topLoser.symbol;
    document.getElementById('topLoserChange').textContent = `${topLoser[returnField].toFixed(2)}%`;
    
    // Most Volatile
    const mostVolatile = [...data].sort((a, b) => b[volatilityField] - a[volatilityField])[0];
    document.getElementById('mostVolatile').textContent = mostVolatile.symbol;
    document.getElementById('mostVolatileValue').textContent = `${mostVolatile[volatilityField].toFixed(2)}%`;
    
    // Highest Volume
    const highestVolume = [...data].sort((a, b) => b[volumeField] - a[volumeField])[0];
    document.getElementById('highestVolume').textContent = highestVolume.symbol;
    document.getElementById('highestVolumeValue').textContent = formatVolume(highestVolume[volumeField]);
}

// Render Charts
function renderCharts(data) {
    const topCoins = [...data].sort((a, b) => b[getVolumeField()] - a[getVolumeField()]).slice(0, 10);
...
function renderTable(data) {
    ...
    // Sort by return %
    filteredData.sort((a, b) => Math.abs(b[returnField]) - Math.abs(a[returnField]));
    
    // Render rows
    tbody.innerHTML = filteredData.slice(0, 20).map((coin, index) => `
        <tr>
            <td>${index + 1}</td>
            <td>
                <div class="coin-info">
            ...
            <td>
                <span class="price-change ${coin[returnField] >= 0 ? 'positive' : 'negative'}">
                    ${coin[returnField] >= 0 ? '+' : ''}${coin[returnField].toFixed(2)}%
                </span>
            </td>
